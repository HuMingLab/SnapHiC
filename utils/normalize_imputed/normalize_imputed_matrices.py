...
def main(args):
    ...
    if args.filetype not in ['npz', 'hdf5', 'bedpe']:
        ...
        process_hdf_file(args.infile, args.outdir, args.chrom, binsize = args.binsize, dist = args.dist)
    elif filetype == "npz":
        args.process_npz_files(args.infile, args.outdir, args.chrom, binsize = args.binsize, dist = args.dist)
    else:
        args.process_bedpe_files(args.infile, args.outdir, args.chrom, binsize = args.binsize, dist = args.dist)

def read_imputed_matrix(filename):
    with h5py.File(filename, "r") as impute_f:
        coordinates = np.array(impute_f['coordinates'])
...
def process_hdf_file(input_filename, outdir, chrom, binsize = 1e4, dist = 2e6):
    ...
    for cell_name, mat in cells:
        ...
        mat = mat[remove_bins:-remove_bins, remove_bins:-remove_bins]
