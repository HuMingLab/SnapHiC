...
In addition, for steps 3 and 4, user need to provide sufficient memory per processor. If scHi-C dataset contains 100-500 cells, we recommend providing 20GB of memory per processor. We also recommend not using more than 5 processor per node; since step 3 involves simultaneous access to multiple files, depending on your system, there might be a limit on the maximum number of files allowed to be open at the same time. By breaking down the computation into these two parts, user can optimally adjust the memory and processor per node according to the HPC resource and the number of cells in scHi-C dataset.  
>>>>>>> 6a773cf2f72dfc267fc47457495051b4ba84eb49
